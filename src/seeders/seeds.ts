...
async function main() {
  ...
  const utilisateur3 = await prisma.user.create({
    data: {
      nom: "Mamadou Fall",
      login: "MamadouFall",
      password: passwordHash,
  ...
  const utilisateur4 = await prisma.user.create({
    data: {
      nom: "Awa Diop",
      login: "AwaDiop",
      password: passwordHash,
  ...
  const utilisateur5 = await prisma.user.create({
    data: {
      nom: "Cheikh Ndiaye",
      login: "CheikhNdiaye",
      password: passwordHash,
  ...
  const tacheAstou = await prisma.taches.findFirst({ where: { userId: utilisateur2.id } });
  if (tacheAstou) {
    await prisma.permissionUserTache.createMany({
      data: [
  ...
  const tacheSoda = await prisma.taches.findFirst({ where: { userId: utilisateur1.id } });
  if (tacheSoda) {
    await prisma.historiqueModifTache.createMany({
      data: [
