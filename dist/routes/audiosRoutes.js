...
router.post("/audio", AuthMiddleware.authenticateUser, audioUpload.single("audio"), (req, res) => {
    if (!req.file) {
...
router.get("/", AuthMiddleware.authenticateUser, (req, res) => {
    fs.readdir(audioDir, (err, files) => {
        if (err)
            return res.status(500).json({ error: "Impossible de lire le dossier audio" });
        res.json({ audios: files.map(f => `/uploads/audios/${f}`) });
    });
});
export default router;
//# sourceMappingURL=audiosRoutes.js.map